...
class VKApi(object):
    ...
    def get_user_info(self):
        user_data = self.api.users.get(fields=['photo_200', ])[0]
        return {
            'uid': user_data['id'],
...
class FacebookApi(object):
    def __init__(self, token):
        self.api = facebook.GraphAPI(access_token=token, version="2.12")

    def get_user_info(self):
        user_data = self.api.get_object('me', fields=[
            'id', 'first_name', 'last_name', 'cover'
        ])
        return {
            'uid': user_data.get('id'),
